...
public class APKFileReader {
	...
	public static PackageInfo getProperties(String apkPath, Activity parent) {
		
		APKFileProperties toRet = new APKFileProperties();
	...
	public static String getManifestFile(String apkPath) {
		
		File apkFile = new File(apkPath);
		if (apkFile.exists() && apkFile.canRead()) {
			try {
				JarFile jFile = new JarFile(apkFile);
	            InputStream is = jFile.getInputStream(jFile.getEntry("AndroidManifest.xml"));
	            byte[] xml = new byte[is.available()];
	            int br = is.read(xml);
	            
	            String xmlResult = decompressXML(xml);
		...
		return "File does not exists or can not read";
		
	}
	
	public static String getManifestFileString(String apkPath) {
		...
		if (apkFile.exists() && apkFile.canRead()) {
			try {
	            ...
	            String xmlResult = new String(xml, "UTF-8");
	            return xmlResult;
...
//	        resultXml.append(prtIndent(indent, "</"+name+">  (line "+startTagLineNo+"-"+lineNo+")"));
	        ...
	        break;
	          ...
	          Log.e("DECOMPRESS ", "  Unrecognized tag code '"+Integer.toHexString(tag0)
	          +"' at offset "+off);
	        break;
	      }
	    } // end of while loop scanning tags and attributes of XML tree
	    Log.i("DECOMPRESS", "    end at offset "+off);
